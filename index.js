...
const downloadJobLogs = async (
  ...
  try {
    ...
    if (!logResponse.ok) {
      ...
      verboseLog(
        `Job ${jobName} (${jobId}): Response code ${logResponse.status}`,
      );
      return false;
    }
 ...
 * @returns {Promise<boolean>} - Whether job logs were downloaded successfully
 */
const downloadAllJobLogs = async (
  host,
  projectId,
  token,
  pipelineId,
  ...
  try {
    ...
    if (logsDownloadedCount > 0) {
      genericLog(
        `Successfully downloaded logs from ${logsDownloadedCount} jobs`,
      );
      return true;
    } else {
...
async function run() {
  ...
  const downloadArtifactsOnFailure =
    core.getInput("download_artifacts_on_failure") === "true";
  ...
  try {
    ...
    if (downloadArtifactsFlag && jobs) {
      ...
      if (finalStatus === "success") {
        ...
        shouldDownload = true;
        downloadReason =
          "Pipeline failed but artifacts download on failure is enabled";
      }
      ...
      if (shouldDownload) {
        ...
        genericLog(
          `Pipeline status is ${finalStatus}, skipping artifact download`,
        );
